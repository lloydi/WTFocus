...
function WTFocus() {
  ...
  function addPanelStyles(WTFpanelWidth) {
    ...
    consoleStyle.textContent =
      "#WTFocusPanel.error {background:darkred;} #WTFocusPanel.warning {background:#CC3300;} #WTFocusPanel[hidden] {display:none;} #WTFocusPanel {border:2px solid #fff;z-index:1000;text-shadow:none;font-family:sans-serif;display:block;text-align:left;position: absolute;z-index:10000;background: black;padding: 20px 20px;width:" +
      WTFpanelWidth +
    ...
    focusable.addEventListener("focus", () => {
      ...
      if ((accName === "")||isDupeAccName) {
        ...
        if (isDupeAccName && (accName !== "")) {
          ...
          log("Accessible Name: ", accName, style_bad_formatting);
