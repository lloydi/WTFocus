...
function WTFocus() {
  ...
  function addPanelStyles(WTFpanelWidth) {
    ...
    consoleStyle.textContent =
      "#WTFocusPanel.error {background:darkred;} #WTFocusPanel[hidden] {display:none;} #WTFocusPanel {border:2px solid #fff;z-index:1000;text-shadow:none;font-family:sans-serif;display:block;text-align:left;position: absolute;z-index:10000;background: black;padding: 20px 20px;width:" +
      WTFpanelWidth +
      "px;font-size:16px;} button {font-weight:bold;background:none;color:#fff;padding:3px 10px;font-size:14px;border:1px solid #fff;display:inline-block;margin:10px 0 -10px 0;} #WTFocusPanel ul,#WTFocusPanel li {margin:0;padding:0;list-style:none} #WTFocusPanel li {margin:3px 0;background:#fff;color:#333;padding:2px} #WTFocusPanel li.outline {outline:4px solid rgb(58, 190, 58);outline-offset:-4px;padding:8px} #WTFocusPanel.error:before {background:darkred} #WTFocusPanel:before {content:'';display:block;height:20px;width:20px;transform:rotate(45deg);position:absolute;background:#000;left:-12px;top:3px;border:2px solid #fff;border-right:none;border-top:none;} #WTFocusPanel.toBottom:before {top:auto;bottom:3px} #WTFocusPanel.toLeft:before {left:auto;right:-12px;border:2px solid #fff;border-left:none;border-bottom:none;} #WTFocusPanel:after {content:'WTFocus';position:absolute;bottom:20px;right:20px;color:silver;font-size:14px;}";
  ...
  Array.from(focusables).forEach(function (focusable) {
    ...
    focusable.addEventListener("focus", () => {
      ...
      if (accName === "") {
        ...
        log("Accessible Name: ", accName, style_good_formatting);
